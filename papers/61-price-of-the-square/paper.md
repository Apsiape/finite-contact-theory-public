# Chapter 61 — The Price of the Square: Merge Geometry, the Retention Bridge, and the One Import

*Finite Contact Theory. The Born rule has a square in it, and for a long
time we wanted the floor to force that square. This chapter is the record
of four audits that took the question apart and put it back together
honestly. The first shows that the floor does not force the square at all:
for every exponent there is a fully coherent world with the full range of
interference, and the cocycle that would tell them apart is blind to the
exponent. The second shows that the exponent is gauge — the worlds are
exactly isomorphic at unit overhead — so the resource-separation angle we
were chasing is dead, and it dies in print. The third classifies every
axiom of the standard reconstruction programs as an import, one explicit
expansion pair at a time. The fourth audits the one place the program has
real derivational muscle — the retention arc — and finds that it earns the
metric but not the linearity: the quaternionic form cI₄ is derived given a
real-linear receiver, and the receiver is admitted. A fifth audit tries to
recover the missing calibration from receipt capacity and fails on a clean
no-natural-ruler theorem. The chapter closes on the pattern all four share:
one meta-import — the linear continuum — with four faces. Inside the
completion the theory is forced-consistent standard quantum mechanics; the
only lawful place a divergence can still live is the finite-native versus
continuum-completed boundary. We name the first candidate world and stop.*

## A note on altitude, first

This chapter retires a divergence we hoped for. That is the honest headline,
put at the top so nothing below reads as consolation. We wanted the
parallelogram law — the amplitude-addition geometry that makes the Born square
inevitable — forced by the floor's own merge structure. It is not. We wanted
the deformed worlds to differ from ours by a measurable resource cost. They do
not; the difference is a coordinate. We wanted the operational reconstruction
axioms to be shadows the floor already casts. They are imports, and we exhibit
the two models that prove it for each one.

What survives is smaller than what we hoped for and more solid than what we
had. The retention arc genuinely earns the quaternionic metric — the unique
invariant form is cI₄, a derivation, not a posit — but it earns it *inside* a
real-linear receiver whose linearity is a hypothesis stated in one exact line
of the argument, and we mark that line. The metric is earned; the linearity
is received. Everything downstream of the linearity — the phase circle, the
radial ruler, the divisibility of coefficients — turns out to be the same
import wearing four costumes. So the chapter's real contribution is not a new
number. It is a boundary, drawn with a proof for each crossing, and worth
more to us than a divergence claimed on one. We register where the
divergence, if it exists, must live.

Labels here are load-bearing: DERIVED (forced by the floor or a stated prior
theorem), EXTENSION (a floor-level result beyond the classical incumbent),
RECOVERY (known physics reconstructed on a stated branch), CONDITIONAL
(DERIVED given a named hypothesis), RECEIVED (the floor stages the fork and
forces no selector), ADMITTED (put in by hand, said so), KILLED (a bet scored
and lost). Several of them are the result, not decoration.

## 1. Merge geometry: the square is not selected

### 1.1 The deformed carriers

Fix the smallest arena the question needs: a source, a two-way fork, a phase
event on one arm, a recombiner, and the receipts the fork and recombiner
write. This is a floor reduct — every object in it is something the floor
already stages — and we will run the whole audit inside it so that nothing
smuggles in structure the floor does not have.

On this reduct define, for each exponent α > 0, a carrier coordinate

    Φ_α(r, φ) = r^{α/2} · e^{iφ}.

Φ_α transports ordinary complex addition through a deformed radial gauge:
magnitudes combine after being raised to the power α/2, phases combine as
phases. The claim — Track J, Theorem 2, the C_α worlds — is that for
*every* α > 0 this is a fully coherent carrier. Concretely it carries:

- abelian coherent recombination (the merge is commutative and associative);
- U(1) phase automorphisms (a global phase rotation is a symmetry);
- scaling distributivity in the deformed coordinate;
- and a genuine interference law with the full fringe range.

The interference law is

    I_α(r, s; θ) = ( r^α + s^α + 2 (rs)^{α/2} cos θ )^{1/α}.

Read the corners. At θ = 0 the two arms are in phase and the intensity is
maximal; at θ = π they are out of phase and

    I_α(r, s; π) = ( r^α + s^α − 2 (rs)^{α/2} )^{1/α} = | r^{α/2} − s^{α/2} |^{2/α},

which is exactly zero when r = s. Perfect destructive interference exists in
*every* one of these worlds, not only in ours; the fringe sweeps its full
range as θ runs over the circle, for every α. This is the first thing the
audit kills: **visibility does not imply the parallelogram law.** The tempting
argument "the arms can perfectly cancel, therefore amplitudes add like
vectors, therefore the norm is Euclidean, therefore the square" fails at every
step after the first — the C_α worlds cancel perfectly and are not Euclidean
for α ≠ 2. Track J Theorem 1 is the countermodel schema: continuously variable
interference, up to perfect cancellation, attaches to *any* continuous
symmetric merge. Interference is cheap; geometry is what costs.

Our own world is α = 2: there I₂(r, s; θ) = √(r² + s² + 2rs cos θ), the law
of cosines, amplitude addition. It sits in the family as one member and
nothing internal to the family points at it.

### 1.2 The cocycle cannot see α

The natural next move is cohomological. The phase data of a merge geometry
lives in a second cohomology — the class [ω] ∈ H² that records how phases
fail to be globally trivializable across the fork. If that class distinguished
α = 2 from its neighbors we would have a forcing.

It does not. The radial conjugacy that carries the α-world to the β-world,

    E_{α→β} : r ↦ r^{α/β},   phases fixed,

commutes with the central phase action. It is a radial reparametrization and
the phase cocycle lives in the central (phase) part, on which E acts as the
identity. So **the H² cocycle cannot see α** (Track J, the H²-invariance
theorem). Every C_α world carries the same phase class. Whatever selects the
square, it is not the cohomological phase data, because that data is constant
along the entire family.

### 1.3 Standard regularity buys the family, not the member

Suppose we demand everything an analyst would call regularity of the merge:
associativity, commutativity, homogeneity (consistent scaling), and an
Archimedean condition (no infinitesimal or infinite magnitudes). These are
strong, but what they cut the space of merges down *to* is the whole power
family {I_α : α > 0}, not its quadratic member. Track J is explicit: standard
regularity selects the power family and never picks out α = 2 within it. The
square is one point in a one-parameter family that every regularity axiom we
know treats symmetrically.

So if α = 2 is to be selected, the selector must be something *outside* merge
regularity. Track J finds exactly two such selectors, and shows both are
imports.

### 1.4 The two selectors, and why both are imports

**Selector A — linear same-ray scaling.** Demand that combining λ copies and
μ copies of the same carrier state x behaves additively in the coefficients:

    (λ + μ) · x = λ · x [+] μ · x,   equivalently   I(r, s; 0) = r + s.

That is, in-phase intensities add linearly on a single ray. This picks α = 2
immediately. But the demand is not in the floor language. It identifies the
physical magnitude of a carrier with a *vector scalar* and identifies
in-phase coherent recombination with *vector addition*. The floor stages
neither identification. Selector A is ADMITTED.

**Selector B — faithful linear-isometric phase mixing.** Work on a
degree-one normed carrier and demand that mixing over the phase circle be a
faithful, continuous, linear isometry. Average over U(1) and apply the
Jordan–von Neumann theorem (Jordan and von Neumann 1935: a norm satisfies the
parallelogram identity if and only if it comes from an inner product). The
average of the parallelogram identity over the phase group forces the inner
product, hence α = 2. This is a real theorem and we use it later with respect.
But it too is an import: it presupposes a *linear* carrier and an *isometric*
mixing on it, and it presupposes that the operational magnitude is the
degree-one norm of that linear structure. The floor stages a merge; it does
not stage a linear normed space with an isometric group action. Selector B is
ADMITTED.

The Born chain in its first honest form (Track J) reads:

    floor fork + phase + recombination
      + [ADMITTED: linear/isometric carrier calibration]  ⟹ Pythagorean merge
      + [ADMITTED: additive nonnegative weight]            ⟹ f = c·r²
      + [ADMITTED: normalization / selector semantics]     ⟹ Born.

Three admissions, printed. Nothing here is a derivation of the square from
the floor.

### 1.5 Weak versus strong phase covariance

One phrase did real damage before we disambiguated it: "phase covariance." A
merge is *weakly* phase-covariant if a global phase rotation is a symmetry of
it. Every C_α world is weakly phase-covariant — U(1) acts by automorphisms
for all α. A merge is *strongly* phase-covariant if the phase action is by
linear isometries of a normed carrier — which is nearly to say the carrier
already contains Euclidean geometry. Only α = 2 is strongly phase-covariant.
The sleight is to prove weak covariance (cheap, holds for all α), name it
"phase covariance," and quietly spend it as strong covariance (the
conclusion). Track J's sleight audit flags exactly this substitution: held
apart, weak covariance proves nothing and strong covariance is the square in
disguise.

## 2. α is gauge: the divergence angle is dead

We had hoped the deformed worlds were physically distinct from ours — that
running a computation in a C_α world for α ≠ 2 would cost more, or need more
depth, or need ancillas, and that the cost would be a forced resource
inequality and therefore a divergence. Track J-2 is the epitaph for that
hope.

The conjugacy E_{α→β} : r ↦ r^{α/β} is not merely a map; it is an *exact
isomorphism* of the minimal worlds. It preserves phases, preserves the
interference pattern, preserves the path count, preserves depth, preserves
the receipt genealogy. Its overheads are: dimension overhead 1, depth
overhead 1, ancilla overhead 0. **Unit overhead — a relabeling.** At minimal
scope α is a radial coordinate and nothing more: r_β = r_α^{α/β}. Forced
resource separation between the α-worlds: REFUTED. We scored this as a
divergence bet and it lost cleanly.

Losing it taught the instrument a new calibration, and this is the part worth
keeping. The resource-gap doctrine — the standing rule that only *forced
resource inequalities* survive as candidate divergences, because value-level
differences are always forgeable by a relabeling — now carries a mandatory
rider:

> **The coordinate-conjugacy test.** A forced resource inequality counts as a
> candidate divergence only if it is invariant under every admitted
> re-coordinatization of the minimal structure. If a relabeling of the
> carrier coordinate erases the separation, the separation was never
> physics.

This is the fourth doctrine upgrade the resource-gap program has taken, and
its subtlety is new: even a *resource* separation, not merely a value
separation, can be a representation artifact. From here, any claimed
separation must pass the conjugacy test before it is registered. α failed it.

### 2.1 The conditional non-embedding, with its exact scope

There is a true statement in the neighborhood, and we state it with its fence
intact. Inside imported linear-norm semantics — once you have granted a linear
carrier with a norm and ask whether the α-deformed norm ℓ_α² embeds
isometrically and linearly into a Hilbert space — the answer for α ≠ 2 is no.
The parallelogram defect is

    2^{1 + 2/α} ≠ 4   for α ≠ 2,

and infinite dimension does not help. This is a real theorem of linear
simulation theory, and its scope is exactly the imported linear-norm frame: a
statement about a world *plus* a chosen radial-linear structure, not about the
floor. Track L sharpens it: the non-embedding is not even invariant under the
admitted radial conjugacies until an independent linear calibration is fixed,
because the conjugacy is nonlinear with respect to the fixed linear structure
(homogeneity ratio 2^{t−1} ≠ 1). So the non-embedding is CONDITIONAL —
substantive inside linear simulation theory, silent at floor scope.

## 3. The reconstruction axioms are imports, one pair at a time

The operational reconstruction programs — Hardy (2001), Masanes–Müller
(2011), Chiribella–D'Ariano–Perinotti (2011) — derive finite-dimensional
complex quantum theory from a handful of operational axioms. If any of those
axioms were a shadow the floor already casts, we would have a genuine bridge
to quantum theory. Track J-3 checks every one on the fixed floor reduct
(source, two-fork, phase event, recombiner, receipts) and the verdict is
uniform: every operational axiom is ADMITTED, each with an explicit *expansion
pair* witnessing that it is not definable from the floor primitives.

The method is Padoa's (Padoa 1901): to show a notion is not definable from a
set of primitives, exhibit two models that agree on all the primitives and
differ on the notion. Two such models — an expansion pair — is a proof of
independence, not an opinion. For each axiom below we have that pair on one
fixed reduct.

Classified ADMITTED, each with its two-expansion witness:

- convexity
- causality (in the probabilistic, no-signalling-from-the-future form)
...
Every one of these is a place where a reconstruction program reaches past the
floor and puts something in by hand. That is no criticism of those programs —
they are explicit about their axioms — it is a *placement*: the floor sits
*below* the operational axioms, not beside them. What the floor *does* stage,
as derived or staged shadows, is a strictly combinatorial residue:

- finite alternatives (the fork has finitely many arms);
- relabelings (arm and receipt names are gauge);
- the phase cocycle (the H² class of §1.2);
- split/recombine incidence (the combinatorics of which arm meets which);
- receipt ancestry (the genealogy the ledger writes);
- and **acyclic ancestry** — DERIVED — which is the floor's own weak shadow
  of causality: the ledger cannot write a receipt before the event that
  causes it, so ancestry is acyclic, and this is as much of "causality" as
  the floor forces on its own.

Combinatorial shadows, staged. Operational geometry, imported. The table is
long and its lesson is short: the reconstruction axioms are not hiding in the
floor.

## 4. The retention bridge: the metric is earned, the linearity is received

The one place the program has real derivational torque on the carrier is the
retention arc (the non-exact-return construction, NXR). It builds carrier
structure from a single principle — that a one-use discipline forces a
non-exact return, a history that cannot be undone — and it has, at various
times, been read as deriving the quaternions, the invariant metric, and
therefore the Born square outright. Track K is the audit that finds the exact
seam.

### 4.1 What retention derives before any linearity

There is a genuinely pre-linear chain, group-level throughout, with no vector
structure anywhere in it:

1. **Retentive return** (NXR §1): a group map with a retained kernel — the
   return is not the identity, and what it fails to erase is the retained
   history.
2. **Retentive-lift selection** (§2): among the lifts, retention kills the
   split extension A₄ × C₂ and forces the *nonsplit* double cover 2T. This is
   an EXTENSION at floor level — a structural fact the floor forces that goes
   beyond the classical incumbent, and it uses no norm and no vector space.
3. **Dual-receipt identity** (§3): the three imaginary units appear here, but
   strictly as the Q₈ group elements I, J, K. The notation must not be allowed
   to smuggle vector algebra; at this stage they are group generators, nothing
   more.
4. **Spinor genesis** (§4): visible order 2, retentive order 4 — the double
   cover, without any inner product.

All four steps are real, floor-level, and free of linearity. The 2T history
lift in particular is an honest EXTENSION and we carry it as one.

### 4.2 The exact line where linearity enters

Then comes the first linear step, and we state its location to the section
number because the whole audit turns on it. NXR §5, the Primitive Receiver
Theorem, opens: *"let a receiver be real-linear, irreducible, and
scar-faithful."* From that hypothesis it follows that dim_ℝ = 4 and the
receiver algebra is ℍ, the quaternions. This is an IF–THEN. It is not
"retention implies linearity." Retention derives the *group*; the *linearity*
is the antecedent of a conditional.

So the claim "retention derives linearity" is KILLED, and we score it. The
tempting rescue — form the group algebra ℝ[2T] and call the linearization
canonical — fails, because choosing that functor *is* choosing the category:
ℝ versus another semiring, superposition-as-recombination versus something
weaker, group algebra versus homogeneous space, scalars-as-magnitude versus
scalars-as-bookkeeping. None of these is selected by retention; the
linearization is a choice with retention's name written on it after the fact.

### 4.3 The two selectors, re-audited on the retention side

**Selector A (same-ray scaling): KILLED from retention.** To get it you must
identify physical magnitude with the vector scalar *and* identify lawful
co-phased recombination with vector addition. Retention derives neither.
Worse, the convex mixtures retention does produce are *preparation
uncertainty* — classical ignorance about which arm — not coherent addition of
amplitudes. Reading a convex mixture as a coherent sum is a category error the
audit catches.

**Selector B (the norm/isometry half): DERIVED, conditionally.** This is the
real prize. Given the linear receiver, NXR §7 shows the invariant symmetric
form is *unique*: I-, J-, and K-invariance together solve to

    G = c·I₄,

the Euclidean form, up to the single scale c. This is a genuine derivation:
the metric is not posited; the requirement that the three imaginary units all
act by isometries forces the form to be a scalar multiple of the identity in
four real dimensions, which is the Euclidean metric. The scar transformations
are then isometric and the norm is degree-one. The alternative route —
JFG-1's quadratic fold — is *not* an independent confirmation, because its own
hypotheses (degree-2 homogeneity plus a parallelogram law) already contain the
geometry they claim to derive; it assumes what it proves. The clean route is
the invariance solution.

**Continuous U(1) from retention alone: KILLED.** The native phase structure
retention gives you is *finite quarter-turns* — the order-four elements of the
group. The finite limit of that structure is ℚ/ℤ, the rationals mod one, not
the circle S¹. Passing from ℚ/ℤ to S¹ is a completion, and it is ADMITTED
(the retention record's own FR-3.6 says so). The circle is not native; it is
the completion of the native object.

### 4.4 The conditional Selector-B theorem

Assembling the survivors:

> **Conditional Selector-B Theorem.** 2T history lift + an admitted
> real-linear receiver + scar faithfulness + a RECEIVED world polarization J
> (a choice of complex structure from the S² of them — no natural selector,
> matching the lift-type classification of chirality-adjacent data elsewhere
> in the program) + phase-receiver linear calibration ⟹ a faithful,
> continuous, linear-isometric phase plane ⟹ the Pythagorean merge (through
> the Jordan–von Neumann selector of §1.4).

Every hypothesis in that statement is labeled. The polarization J is RECEIVED
— and we note the unplanted convergence that the same "RECEIVED, no natural
selector" verdict on the S² of complex structures appears independently in
the program's lift-type classification, two audits reaching the same fork
from different sides.

### 4.5 The six sleight-kills

The audit ran six attacks against its own derivation and killed all six; each
is a way one could over-claim the retention result:

1. **Ambient-linearity credit** — crediting retention with the linearity
   assumed in §5. The linearity is the hypothesis, not the theorem.
2. **Uniqueness-as-existence** — reading "the invariant form is unique" as
   "forced to exist and to be physical magnitude." Uniqueness of the form
   given the structure does not establish that the structure is the carrier.
3. **The "amplitude" smuggle** — using the word *amplitude* for the group
   object, thereby importing magnitude-and-addition semantics through
   vocabulary. Refused until the semantics is earned.
4. **Invariant-form-as-operational-magnitude** — identifying the derived form
   with the magnitude a receipt reports. The radial identification stays
   ADMITTED and the radial conjugacies stay free; deriving the form does not
   fix which function of it you measure.
5. **Quarter-turns-to-U(1)** — treating the finite native phase group as the
   circle. ℚ/ℤ is native, S¹ is the completion, the completion is admitted.
6. **Kernel-to-probability** — reading the finite 24-cell Born kernel as a
   root derivation rather than a downstream consequence of the already-linear
   formalism. The kernel is a genuine conditional finite recovery, but it lives
   *downstream* of the linear structure and does not reach back to found it.

### 4.6 The corrected Born chain, every link labeled

    one-use retention  ⟹ 2T history lift              [DERIVED — EXTENSION]
    + primitive real-linear receiver                   [ADMITTED = LCC]
    ⟹ ℍ receiver, G = c·I₄                             [DERIVED COND. — RECOVERY]
    + world polarization J                             [RECEIVED (lift-type)]
    + phase U(1)                                       [DERIVED in the phase arc,
                                                        cond. on completion]
    + phase-receiver linear calibration                [ADMITTED BRIDGE]
    ⟹ linear isometric phase mixing ⟹ √(r² + s²)      [DERIVED via selector B]
    + nonnegative additive weight                      [ADMITTED]
    ⟹ f = c·r²                                         [DERIVED]
    + normalization / selector semantics               [ADMITTED]
    ⟹ Born                                             [RECOVERY, CONDITIONAL]

Read the chain as a ledger: the floor pays the first line outright, pays the
fifth (ℍ and the metric) *given* the second, and the rest is a mix of received
forks and admitted bridges. There is exactly one place the linearity enters,
and it is the second line.

### 4.7 The residue, named exactly

The retention audit did not fail to find a derivation; it found a *smaller*
import than we had before, and it named it:

> **Linear Carrier Calibration (LCC): why physical carrier combination is
> linear at all, and why operational magnitude is the degree-one norm of that
> carrier.**

This is genuine progress over the Track J form, whose import was "linear *and*
isometric carrier calibration" — the whole Euclidean package. Retention
*discharges the isometric half*: given LCC, the metric cI₄ is derived, not
posited, so the import shrank from "an arbitrary Euclidean metric" to "the
linearity of combination plus the identification of magnitude with the norm."
What remains is more fundamental and smaller. **The metric is earned; the
linearity is received.** That sentence is the chapter's center.

## 5. The capacity ruler cannot break the gauge

If linearity and the radial identification are the residue, the obvious place
to look for them is the ledger itself. The floor writes receipts; receipts
have a genealogy; maybe the *capacity* of that genealogy is a natural ruler
that fixes the radial coordinate and thereby forces the square. Track L tests
this and it fails on a theorem.

**Pathwise capacity exists.** Define C(p) = Σ ln g over the receipt ancestry
of a path — the sum of the log-branchings along the lineage. This is a real,
well-defined quantity; its existence is a RECOVERY. But it cannot be
identified with the carrier radius, for two independent reasons.

*The countermodel.* There are worlds with the same fork and the same lineage
capacities but *unequal* lawful radii — no uniformity law excludes the
unequal-radius world. Capacity does not determine radius because two worlds
can agree on all capacities and disagree on the radii.

*The deeper obstruction.* Assigning capacity to the radial coordinate would
require each *staged* (coherent, unresolved) alternative to carry a live
resolved receipt lineage. But staged alternatives have no such lineage — they
are precisely the parts of the fork that have not paid out — so giving each
simultaneous alternative its own live capacity would *duplicate ledger
capacity*, which the ledger forbids. Counterfactual-lineage semantics is an
admitted extension, not a floor fact; the identification fails by typing,
independent of any model.

**The gauge computation.** Push the strongest natural bridge as far as it
goes — weights multiply along a path, a symmetric g-fork contributes 1/g,
capacity is branch surprisal — and it yields

    r^α = e^{−C}.

Capacity calibrates the *invariant weight* r^α, the one combination that is
the same in every gauge, and therefore *survives every conjugacy* r ↦ r^{α/β}.
The capacity report is correct in every gauge, which is exactly why it cannot
break one: a quantity invariant under the reparametrization cannot select a
preferred parametrization.

**The no-natural-ruler theorem.** Any α-independent injective ruler Q with
Q(r^{α/β}) = Q(r) forces α = β — a natural injective map from capacity to
radius respecting the admitted conjugacies cannot exist unless it is trivial,
so a gauge-breaking ruler is *necessarily* extra structure. Import a ruler
r = e^{−λC} and α becomes measurable with α = 1/λ — but *every* α is obtainable
this way, and the ledger does not determine λ. The selection of α = 2 via
λ = 1/2 is the answer inserted as the input; the audit flags it as the primary
sleight. Selection of the square from capacity: REFUTED. We scored this branch
too, and the branch we bet against fired cleanly.

The staged alternatives close the loop: the only way to make capacity a ruler
is to give the unresolved alternatives live lineages, and that duplicates the
ledger — the ledger's own conservation forbids the ruler. The capacity is
real, correct, and gauge-natural; it calibrates the invariant weight and
refuses to calibrate anything else.

## 6. The continuum thesis

Four audits, run largely independently, each hunting a different way to force
the square, each landing on an import. Stack the imports and they are not four
things. They are one thing with four faces.

- **Linearity / the reals.** The retention residue LCC: combination is linear
  and magnitude is the degree-one norm. (Track K.)
- **The phase circle.** The floor's native phase object is finite quarter-turns
  whose limit is ℚ/ℤ; the circle S¹ is the completion. (Track K.)
- **Divisible coefficients.** The panel's μ₂-versus-U(1) finding: the floor's
  native coefficient object is a finite two-element sign group, and admitting
  the continuous phase — admitting *i* — *erases* the ℤ₂ rung-4 class that the
  finite object carries. Divisibility of coefficients is not native; it is what
  the completion adds, and it costs a class.
- **The radial ruler.** Track L's λ: fixing the radial coordinate requires an
  imported ruler the ledger does not determine.

Linearity, the circle, divisibility, the radial ruler. Four sightings of a
single meta-import: **the linear continuum.** The floor is finite and
discrete — finite forks, finite quarter-turns, a two-element sign group, a
gauge-free invariant weight — and every route to the Born square passes
through the same completion of that finite structure into a continuum. We did
not design the four audits to converge; they converged, and the convergence
is the result.

### 6.1 The loop closes

The retention audit and the capacity audit close a loop, and the closure is
exact. Track L lists the possible sources of λ = 1/2 — "a degree-one linear
carrier with an independently established quadratic norm, a parallelogram
law." That list is *precisely* what Track K proves retention supplies, *given*
LCC. So the two audits are not merely compatible; they are two ends of one
implication. Inside the linear completion, λ = 1/2 is retention-forced, the
metric is cI₄, the phase plane is Euclidean, and the merge is Pythagorean.
Everything is consistent, forced, standard quantum mechanics. This is
RECOVERY, and it is clean.

The consequence for divergence-hunting is sharp and it is the reason the
chapter can end honestly rather than wistfully. **The divergence does not live
inside the completed theory.** Inside the completion, the theory is
forced-consistent QM; there is nowhere in there for a lawful disagreement with
nature to hide, because every degree of freedom is pinned. The only freedom
left anywhere in the four audits *is the completion itself* — whether to take
it. So the only place a divergence can lawfully live is the boundary:

> **The finite-native vs continuum-completed boundary.** Wherever a
> finite-native structure (μ₂ coefficients, ℚ/ℤ phase, gauge-free invariant
> weight, no radial ruler) makes a prediction that differs from its
> continuum completion (U(1), i, the linear norm, λ = 1/2), that difference —
> and only that difference — is a candidate divergence. Everything interior to
> the completion is recovery.

This is the program's registered forward target from here. Not "force the
square" — the square is not forced, four times over, and we have printed the
proofs. The target is the seam where the floor's finiteness has not yet been
completed away.

### 6.2 The first candidate world

We name one and stop, because naming more without building them would be the
hype this chapter is written against. The panel's own sighting points at it:
admitting i erases the ℤ₂ rung-4 class. So the sharpest place to look for a
finite-native/continuum-completed disagreement is a world built on the
μ₂-native fourth-degree structure — the rung-4 world that keeps the two-element
coefficient group *instead of* completing to U(1), and asks what it predicts
where the completed theory predicts something else. If it and its U(1)
completion agree everywhere, the continuum was free and the thesis is a
bookkeeping remark; if they disagree at even one gauge-free observable, the
boundary is real and the disagreement is the divergence the program has been
trying to force from the wrong side.

That world is not built here. Registering it as the next target — with the
exact reason it is the right one, and the exact form a positive result would
take — is the work of this chapter. The four imports are one import; the one
import is the linear continuum; the divergence, if there is one, lives where
the floor has not yet been completed. We go build the μ₂-native fourth-degree
world.

## Corrections carried

Three claims the program has at times flown are retired here, in print:

1. **"The floor forces the Born square."** Retired. The floor forces neither
   the parallelogram law (§1) nor the radial ruler (§5); the square is a member
   of a gauge family, selected only by admitted structure.
2. **"The deformed exponent is a physical divergence."** Retired. α is gauge at
   unit overhead (§2); the resource-separation reading failed the
   coordinate-conjugacy test, which the failure itself installed as mandatory.
3. **"Retention derives the quaternionic quantum carrier outright."** Corrected,
   not retired. Retention derives the metric cI₄ given a real-linear receiver;
   the linearity is the hypothesis, stated at NXR §5. The result is CONDITIONAL,
   the residue is named (LCC), and the correction *strengthens* the claim by
   shrinking the import.

The kill-count is the health metric. Three retirements and two lost bets, all
scored, all in print. What is left standing — the 2T EXTENSION, the derived
metric, the no-natural-ruler theorem, the four-faced continuum thesis, and a
named forward target — is standing on proofs, not on hope.

## Sources and citations

- P. Jordan and J. von Neumann, "On inner products in linear, metric spaces,"
  *Annals of Mathematics* 36 (1935) 719–723 — the parallelogram
  characterization of inner-product norms, used in Selector B.
- A. Padoa, "Essai d'une théorie algébrique des nombres entiers, précédé
  d'une introduction logique à une théorie déductive quelconque" (1901) — the
  independence-by-two-models method underlying every expansion pair in §3.
- L. Hardy, "Quantum Theory From Five Reasonable Axioms" (2001);
  L. Masanes and M. P. Müller, "A derivation of quantum theory from physical
  requirements" (2011); G. Chiribella, G. M. D'Ariano, and P. Perinotti,
  "Informational derivation of quantum theory," *Phys. Rev. A* 84 (2011)
  012311 — the operational reconstruction programs whose axioms §3 classifies.
