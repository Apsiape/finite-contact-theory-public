# Chapter 22 — The Price of Chance: The Measure Dilemma, the Field Carrier, and the Earned Key

**Release:** v0.19.0 · **Status:** an exact, finite account of where a
branch measure can come from, what each route costs, and what is
forced. Seventeen sections, 52 checks, all exact or exhaustive, in one
shipped verifier (`verification/scripts/price_of_chance.py`). This
chapter carries the live release ceiling (§ below). The route was
constrained in advance by the program's own theorems — the floor has
no natural sampler, forced structure is grammar and never magnitude,
and the Born square is forced only given phase — so this chapter never
hunts a Born-from-counting derivation; it prices the alternatives.

Setting: the static one-use floor of Chapter 19 (branches = maximal
runs; after quotienting by contact order, maximal matchings), plus the
minting genesis floor of Chapter 21 for the final sections.

---

## The live release ceiling

Quoted identically in the README, the claim register, and the v0.19.0
release notes:
...
## 1. The anatomy of time (two arrows, the frozen wound)

The one-arrow hypothesis was registered to die and died: crystals
(self-reproducing worlds) are perpetual clocks — orientation advances
forever while total tolerance never moves. Time factors into a CLOCK
arrow (universal) and a LEDGER arrow (spend). The **frozen-wound
theorem** (exhaustive, zero violations): a contact lazy in both twin
worlds hands any divergence to its offspring pointwise unchanged —
differences cannot spread, move, or heal through equilibrium. All
information dynamics lives on the spend sector.

## 2. What symmetry forces and the seat un-forces

On bare cliques the automorphism group is transitive on branches:
the unique symmetric measure is counting — and forced uniformity is
the refusal of selection, the no-selector law's own measure. But an
observer's seat breaks the forcing: the seat-fixing stabilizer splits
branches into orbits (4/8/9 on K4/K5/K6 against 1/1/1 bare), because
relabeling cannot reorder time. The orbit invariant is the
**participation calendar** — when the observer's marks are spent,
together or apart, and who never spends — with a closed form on
cliques (m² even, m²+2m odd; the K7 and K8 values were predicted
frozen and hit). Received slot-exchangeability (consistent, never
floor-forced; de Finetti's posture recovered as one received key)
collapses the key to interaction-type propensities: not WHEN but
WITH WHOM. **Classification (per the blind sweep):** transitivity
forcing and its loss under conditioning are standard ergodic-theory
structure (Choquet simplices; Phelps cited), and seat-symmetry
arguments are the Everettian literature's (Sebens–Carroll 2018; Elga
2004 cited); the participation-calendar coordinates are the priority
candidate.

## 3. The measure dilemma

- **Affordability:** counting's propensity is provably nonlocal —
  path worlds with identical radius-5 seat neighborhoods have
  different propensities (7/10 vs 44/61 vs 81/110); maximal-matching
  counts are global objects. A received constant is evaluable from
  the seat's own trace; counting is not.
- **Composition:** no internal filter forces the constant's value —
  no-signaling composition, growth stability, and exchangeability
  admit every value.
- **Intersubjectivity:** two observers holding the same private
  constant agree about jointly definable events ONLY at the counting
  values (1/5 on K6, 1/7 on K8, all six seat geometries unanimous,
  hand-derived frozen and hit).

**The measure dilemma (theorem at model scope): a weighting can be
SHARED or AFFORDABLE, never both.** Private constants disagree —
agreement on co-witnessed events (Chapter 19's no-Rashomon) stops
exactly at weights — and the unique consensus weighting cannot be
computed by any bounded insider. **Classification:** every ingredient
is named classical art (local lower bounds for maximal matchings:
Kuhn–Moscibroda–Wattenhofer, Balliu et al.; agreement: Aumann,
Aaronson; the uncomputable canonical prior: Solomonoff/Levin,
Li–Vitányi) — the sweep found the dichotomy *theorem* stated nowhere;
it is the chapter's first priority claim, a new organization of a
firmly known tension.

## 4. The field carrier (the dilemma dissolved, at a price)

On path worlds the order factor (each maximal matching realized by
exactly |M|! runs — a global, non-factorizable weight) is precisely
what the received time-symmetry deletes; what remains is carried by a
three-dimensional strictly local recurrence M(n) = M(n−2) + M(n−3),
with edge propensities factorizing as M(k)M(n−k−2)/M(n). One
propagated field serves every seat: both observers' routes
P[A]·P[B|A] and P[B]·P[A|B] coincide exactly with brute force. **The
dilemma dissolves at the price of upgrading the received key's type:
constant → field** (per-site finite-dimensional state, a local update
rule, one boundary condition) — the floor-level shape of a wave
function, as an existence proof at model scope; no Hilbert space is
claimed. **Classification:** the recurrence, the factorization, and
exactness-on-trees are classical (Heilmann–Lieb 1972; Pearl 1988;
Mézard–Montanari 2009; and Bonte–Devillez–Hertz–Mélot 2024, the most
directly overlapping work, covering exactly these path/cycle
recurrences and their asymptotics) — all cited as the mathematics;
the measure-carrier framing and the dilemma-dissolution role are the
chapter's layer.

## 5. Forced limits, the boundary layer, and the closed world

In the large-world limit the field's ratios become world-size
independent and their values are forced algebraic numbers of the
recurrence: the end-edge propensity converges geometrically (envelope
0.658 = |λ₂/λ₁|) to 1/ρ² ≈ 0.5698 (ρ the plastic number), the deep
bulk to C/ρ² ≈ 0.4115 (C the Padovan amplitude). The propensity
profile is a **boundary layer of chance** (decay length ≈ 2.4 sites);
a cycle — a boundaryless world — sits at the bulk value everywhere
(the bulk identity, to float precision), converges at twice the open
world's rate, and its branch counts are the Perrin numbers, computed
by the TRACE of the same local law that computes the open world by
boundary products: closing the world replaces the seed by
self-consistency. On prime cycles rotation acts freely on branches
(the Perrin prime-divisibility as the counting witness): the uniform
measure is exactly symmetric while every branch is completely
asymmetric — the spontaneous-symmetry-breaking structure with
breaking MANDATORY on primes. **Classification:** the plastic-number
asymptotics, Perrin identification, boundary-effect machinery, and
the free-action divisibility proof are KNOWN combinatorics and
statistical mechanics, cited as recovery (Bonte et al. 2024;
Bisdorff–Marichal 2008; Füredi 1987; Lucas 1878; Georgii 1988 for
extremal decomposition); the boundary-layer-of-chance and
pure-bulk-closure readings, and the doctrine placement (magnitudes
forced GIVEN anchors), are the chapter's layer.

## 6. The earned key (the crystal clock, the mortality bound and bias)

On self-reproducing (genesis-floor) crystals, an observer's
participation is exactly i.i.d. at rate 2/m: the mean waiting time is
m/2 — **the world's size is written in the observer's proper time**;
what the dilemma proved unaffordable through space arrives free
through retention. Learnability is exact: the posterior over world
size concentrates at the KL rate (0.0204 nats/contact for adjacent
sizes — slow but unbounded). The **mortality bound** (a frozen bet
killed and upgraded twice, scored): (a) a clique lifetime IS a census
— the mortal trace distributions of different world sizes have total
variation 1, "you learn the size of your world by dying in it"; (b)
but one life provably cannot identify the MEASURE — distinguishing
two weightings from a single life has an irreducible error floor
(0.44 on the model) while the eternal observer's distinguishability
goes to 1. **One life reads the world's structure but never its
weights; measures need repetition, and only eternity repeats.** And
the **mortality bias**: the same event (first encounter = partner?)
has different chances in mortal (1/(m−1) even, 1/m odd) and eternal
(1/(2m−3)) worlds, ratio → 2 — mortality bends chance.
**Classification:** renewal inference, Le Cam two-point bounds,
Chernoff–Stein rates, the inspection paradox, and the finite-
frequentism critiques are the named classical components (Ruggles–
Brodie 1947; Tsybakov 2009; Cover–Thomas; Hájek 1997/2009; Ćirković–
Sandberg–Bostrom 2010 all cited) — the sweep judged the
structure-vs-measure split and the mortal/eternal chance pair the
chapter's second priority claim: a *priced* frequentism.

## 7. The ladder

Three ways to hold a measure, all proven at model scope: a RECEIVED
CONSTANT (price nothing; private; provably disagreeing), a RECEIVED
FIELD (price: boundary data plus a local rule; shared; values forced
in the limit), an EARNED STATISTIC (price: time, via retention;
converging to truth; denied to mortals). Each rung buys agreement
with a different currency, and the currencies are the program's own
primitives: keys, boundaries, retention. The closing suggestion —
that quantum amplitude is shaped like the received COMPRESSION of
what an eternal observer would learn, what a finite observer must be
given because it cannot live long enough — is a target for future
work, not a result; its nearest incumbent is named (Müller 2020,
"Law without law," where objective law emerges as compressed
observer-statistics asymptotics; QBism and the Lewis chance
literature cited for the rungs).

## 8. Scope and fences

All results are exact at their stated finite scopes; the crystal
reference measure is a declared choice whose per-step weight freedom
the retention results do not touch (stated as its own check). The
combinatorial spine is known art and cited as such; the claims are
confined to the participation-calendar coordinates, the
shared-or-affordable dilemma, the field-carrier dissolution and its
type-upgrade price, the boundary-layer/pure-bulk readings, the
mortal/eternal split, and the cost-structure ladder. Nothing about
nature is claimed; whether nature's chance is field-carried or
earned is left open by name in the ceiling.
